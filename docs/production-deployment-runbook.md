...
- `*.sh`、`*.service`、`*.timer` 由仓库属性固定为 LF 行尾；发布包必须从 Git 提交或 Linux 工作区生成，不能从任意 Windows 文件副本直接打包。
- 如果**当前旧 release** 的备份脚本因历史 CRLF 行尾出现 `pipefail` 错误，不要修改正在运行的 release。可仅在本次执行时转换其输入：

  ```bash
  sed 's/\r$//' "${current}/deploy/ops/fenxiao-mysql-backup.sh" | sudo bash
  ```

  这只运行规范化后的脚本文本；下一 release 应由带 LF 属性的 Git 发布包替换该历史副本。
...
当 Linky 收入事实同步收到 `HTTP 410` 且 MCN 确认原因为 `cursor_invalid_or_expired` 时，不得继续自动重试旧游标、猜测游标或清理已入库事实。

1. 系统将同步状态标为 `CURSOR_EXPIRED` 并暂停该平台的自动拉取，旧游标仅保留在受控数据库用于审计。
2. 具备 `FINANCE` 权限的管理员调用 `POST /admin/income-facts/linky-cursor-recovery/probe`，提交一个由 MCN 确认可读取的业务日期。系统使用 `cursor=null`、日期限定、最大 500 条分页读取直到终页；只有所有页面均为 `READY` 且 `FINAL` 才会通过。
3. 探针得到的新增事实会自动进入本地事实投影、公司业务收入、直邀 10%／间邀 3%、2% 预留、有效用户、积分和等级计算；不创建正式奖励、钱包、提现或付款。
4. 对账通过后，具备 `FINANCE` 权限的管理员调用 `POST /admin/income-facts/linky-cursor-recovery/resume`。系统才会将持续流游标重置为 `null`，由定时任务从头建立新的无日期范围修订流。
5. 操作日志只记录状态、日期和计数，绝不返回或记录明文游标、密钥、账号明细或收入事实内容。
