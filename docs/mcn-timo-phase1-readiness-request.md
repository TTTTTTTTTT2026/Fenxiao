...
仅当 Timo 账号属于用户归属国家对应的目标公会，且官方正式入会日期与 BANDEIRA 的账号绑定提交日期绝对相差不超过 1 个自然日时，才允许通过绑定核验。

如果日期相差超过 1 个自然日、无法提供正式入会时间、属于其他目标公会，或存在多公会冲突，BANDEIRA 均视为不通过或进入人工复核。首期不调整为“只要当前在目标公会即可通过”的宽松规则。
...
1. `found / 可通过` 样本
   ...
   该样本需能与 BANDEIRA 预先约定的绑定提交时间满足“相差不超过 1 个自然日”的规则。
