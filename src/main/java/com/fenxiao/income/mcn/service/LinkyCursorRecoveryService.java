...
public class LinkyCursorRecoveryService {
    ...
    public LinkyCursorRecoveryService(McnIncomeFactsClient client, McnIncomeFactsProperties properties,
                                      ...
                                      McnIncomeRawLedgerEventRepository rawEvents, OperationAuditLogRepository audits,
                                      ApplicationEventPublisher events, Clock clock) {
        this.client = client; this.properties = properties; this.rawLedger = rawLedger; this.checkpoints = checkpoints;
        this.rawEvents = rawEvents; this.audits = audits; this.events = events; this.clock = clock;
    }
    ...
    public LinkyCursorRecoveryResponse probe(LinkyCursorRecoveryProbeRequest request, AdminSessionService.AdminPrincipal actor) {
        ...
        McnIncomeFactsReconciliationResult reconciliation = client.reconcile(new McnIncomeFactsReconciliationQuery(PLATFORM, date, date, List.of()));
        Comparison comparison = reconciliation.page().isReady() && isFinal(reconciliation.page().sourceWatermark())
                ? compare(reconciliation.page()) : new Comparison("STALE", 0);
        boolean ready = "MATCHED".equals(comparison.status());
        if (ready) {
        ...
        audits.save(OperationAuditLog.create(actor.accountId(), actor.role(), MODULE, "mcn_income_sync_checkpoint", 0L,
                "PROBE_FINAL_DATE", before, snapshot(checkpoint) + ";date=" + date + ";pages=" + pages + ";new=" + added + ";reconciliation=" + comparison.status(),
                null, ready ? "日期限定读取与对账完成；未重置持续同步游标，未创建奖励或付款" : "日期限定读取完成但对账未通过；持续同步保持暂停", LocalDateTime.now(clock)));
        return new LinkyCursorRecoveryResponse(ready ? "RECOVERY_PROBE_PASSED" : "RECOVERY_PROBE_MISMATCH", date, pages,
                received, added, duplicates, unmatched, comparison.status(), comparison.mismatchGroupCount(), ready);
    }
    ...
    public LinkyCursorRecoveryResponse resumeContinuousSync(AdminSessionService.AdminPrincipal actor) {
        ...
        return new LinkyCursorRecoveryResponse("RECOVERY_RESUMED", null, 0, 0, 0, 0, 0, "NOT_APPLICABLE", 0, false);
    }
    ...
    private Comparison compare(McnIncomeFactsReconciliationPage page) {
        Map<String, Aggregate> expected = new HashMap<>();
        page.groups().forEach(group -> expected.put(key(group.businessDate(), group.guildId(), group.settlementStatus(), group.amountUnit(), group.currencyCode()), new Aggregate(group.factCount(), group.absoluteAmountTotal())));
        Map<String, Aggregate> actual = new HashMap<>();
        rawEvents.findLatestBySourceSystemAndPlatformCodeAndBusinessDateBetween("MCN", PLATFORM, page.businessDateFrom(), page.businessDateTo()).forEach(event ->
                actual.merge(key(event.getBusinessDate(), event.getGuildId(), event.getSettlementStatus().name(), event.getAmountUnit(), event.getCurrencyCode()), new Aggregate(1, event.getAmount()), Aggregate::add));
        int mismatches = 0;
        for (String key : java.util.stream.Stream.concat(expected.keySet().stream(), actual.keySet().stream()).distinct().toList()) {
            Aggregate left = expected.get(key), right = actual.get(key);
            if (left == null || right == null || left.count() != right.count() || left.amount().compareTo(right.amount()) != 0) mismatches++;
        }
        return new Comparison(mismatches == 0 ? "MATCHED" : "MISMATCH", mismatches);
    }
    ...
    private record Aggregate(int count, BigDecimal amount) { Aggregate add(Aggregate other) { return new Aggregate(count + other.count, amount.add(other.amount)); } }
    private record Comparison(String status, int mismatchGroupCount) { }
